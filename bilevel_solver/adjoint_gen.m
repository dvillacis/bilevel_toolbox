...
%   FIDSOL - 1 if fidelity weight is unknown, zero otherwise
...
function [p, gp]=adjoint_gen(A,K,Bs,n,u_fid,u_reg,gamma,z,yn,y,FIDSOL,REGSOL, eta)
    m=n;
    g=1;
    ...
    for i=1:length(Gs)
        ...
        mk=(g*act+Sact*(g-c/2*(g-ga*nGy(1:m^2)+1/(2*c)).^2))./denominador;
        Dmi=spdiags(kron(ones(N,1),mk+ga*inact),0,sz,sz);

        %Termino negativo en la derivada
        subst=spdiags(g*act+Sact*(g-c/2*(g-ga*nGy(1:m^2)+1/(2*c)).^2),0,m^2,m^2);
        ...
        sk2=(Sact*ga^2*(g-ga*nGy(1:m^2)+1/(2*c)))./(denominador.^2);
